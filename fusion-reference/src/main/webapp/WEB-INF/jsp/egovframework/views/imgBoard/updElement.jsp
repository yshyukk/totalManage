		...
		<div id="hashTag_container" class="mb-3">
			...
			<button type="button" id="ins_hashTag">입력</button>
			 ...
			 <div id="show_hashtag" class="hashTag form-control" style="display:inline-block; border:1px solid lightgray;">
				<ul id="hashTag_list" style="display:flex; list-style:none;">
					<!-- 태그 보여줄 부분 -->
					<c:forEach items="${hashTag}" var="hashTag">
						<li class='hashTag_item' style="margin-right:15px">
							<span class='span_hashTag' style="font-size:20px;">${hashTag.tag }</span>
							<button type='button' class='del_btn' style="background-color:transparent; border:none" >x</button>
						</li>
					</c:forEach>
				</ul>
			</div>
       	...
       	<div id="upload_container" class="form-label" >
            <div class="mb-3" >다운로드
            	<ul style="list-style:none" id="img_container">
             	<c:forEach items="${imgInfo}" var="imgInfo">
             		<li class='ins_preimg'  style="font-size:20px; margin-bottom:10px; display:flex; display:inline-block">
						<img src="${imgInfo.imgSavePath}/${imgInfo.imgSaveName}">
						<div style="display:flex; flex-direction:column;">
							
							<a href="${imgInfo.imgSavePath}/${imgInfo.imgSaveName}" download style="text-decoration:none; font-size:15px; margin-top:10px;">${imgInfo.imgSaveName}</a>		                    			
							<input type="hidden" value="${imgInfo.imgId }" name="origin_imgId">
							
	             		    <button type="button" class="btn_del btn btn-outline-danger" id="btn_del" style="margin-left:10px;  margin-top:15px">삭제</button>
	           		        <button type="button" class="btn_imgUpd btn btn-outline-primary"  style="margin-left:10px;  margin-top:15px">변경</button>
	             		    ...
	             		    <div class="pre_container">
		             			...
		             			<%--이미지 프리뷰 --%>
		             			<input class="preview_input" type="file" name="updateImg" style="display:none">
...
$(document).ready(function(){
	...
	$('.btn_imgUpd').click(function(){
		//$(this).next().click();
		$(this).next().find($('.preview_input')).click();
	})
	
	//미리보기 및 업데이트 이미지 정보
	$('.preview_input').on("change", function(e){
		let preContainer = $(this).parent();
		preContainer.append("<img class='preview'>")
		console.log("aaaa",preContainer.find($('.preview')).length)
		if (preContainer.find($('.preview')).length > 1 ){
			preContainer.find($('.preview')).last().remove()	
		}
		
		
		let showPre = $(this).next();
		let imgId = $(this).parent().parent().find('input[name=origin_imgId]').val();
	...
	//첨부파일 등록창 제한
	$(document).on("click", ".remove_fileInfo", function(e){
	    if($('.ins_img').length > 1){
	        $(this).parent().remove();
	    }    
	})
	...
	$(document).on("click", ".add_file", function(e){
		let insPreimgleng = $('.ins_preimg').length;
		console.log(insPreimgleng);
		
		if($('.ins_img').length + insPreimgleng < 5){
	...
	console.log("imgId",originImgIdArr)

	
	
	//console.log("몇개?::::", $('.preview').length)
