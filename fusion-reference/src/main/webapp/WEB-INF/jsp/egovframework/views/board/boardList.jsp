	...
	<main class="mt-5 pt-5">
		...
		<div class="upper">
			<div class="upper_left">
				<form name="searchContainer" method="get" action="/board/boardList.do">
					<input type="hidden">
		...
		<nav aria-label="...">
			<ul class="pagination pagination-lg" >	
				<c:if test="${(param.viewPage != 1)}">
					<li class="page-item">
						<a class="page-link"
		   	   			   href="../board/boardList.do?viewPage=${(param.viewPage-1) }&search=${(param.search)}&searchText=${(param.searchText)}&pageFilter=${(paging.pageFilter)}">Previous</a>
					</li>
				</c:if>
				<c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage }">
					<li class="page-item"><a class="page-link"
						href="../board/boardList.do?viewPage=${i }&search=${(param.search)}&searchText=${(param.searchText)}&pageFilter=${(paging.pageFilter)}">${i }</a>
					</li>
				</c:forEach>
				<c:if test="${(param.viewPage != paging.totalPage) }">
					<li class="page-item"><a class="page-link" 
						href="../board/boardList.do?viewPage=${(param.viewPage+1) }&search=${(param.search)}&searchText=${(param.searchText)}&pageFilter=${(paging.pageFilter)}">Next</a>
					</li>
				</c:if>
			</ul>
		</nav>
...
<script type="text/javascript">
	...
	let accessRole = urlParams.get('accessRole');
	...
	function openIns(){
		window.open(
	       	'/board/boardRegister.do?accessRole='+accessRole+'&menuId='+menuId+'&boardNum='+boardNum,
	       	'opneIns',
	       	'resizable=yes'						    		       	
		)
	};  		
